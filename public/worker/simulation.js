...
function pay_taxes(brutto_amount, tax_rate) {
    var taxes = Math.max(0.0, brutto_amount) * tax_rate;
    var netto_amount = brutto_amount - taxes;
    return netto_amount;
}
...
var HR_TO_ANUAL = 8 /* business hours */ * 20 /* business days */ * 12 /* business month */; // 1920 business hours per year

function simulate_founding_period(input) {
    var sim_data = clone(input);
    var total_founds = sim_data.initial_founds;
    var total_invest = sim_data.initial_invest;
    var financial_independence = sim_data.financial_independent;
    var failed = undefined;
    for (var year = sim_data.start_founding_year; year < sim_data.founding_period_years; year++) {
        var inflation_year = forecast_inflation_rate(sim_data.avg_anual_inflation_rate, year);
        // earn active income by work           
        if (financial_independence === undefined) {
            var income_year = (sim_data.adj_hr_by_inflation ? sim_data.hour_rate * inflation_year : sim_data.hour_rate) * HR_TO_ANUAL;
            total_founds += pay_taxes(income_year, sim_data.income_tax);
        }
        // earn passive income by investments
        var invest_returns_year = calculate_invest_retruns(total_invest, sim_data.avg_anual_invest_return);
        total_founds += pay_taxes(invest_returns_year, sim_data.invest_tax);
        // do anual spending on expenses            
        var expenses_year = sim_data.expected_min_anual_expenses * inflation_year;
        if (expenses_year > total_founds) {
            ...
            var diff = expenses_year - total_founds;
            var taxes = diff * sim_data.invest_tax;
        ...
        if (sim_data.anual_saving_rate > 0.0 && financial_independence === undefined) {
            ...
            var invest_year = sim_data.anual_saving_rate * sim_data.saving_risk_ratio;
        ...
        if (financial_independence === undefined) {
            var sim_data_clone = clone(sim_data);
            ...
            var total_remaining_founds = simulate_founding_period(sim_data_clone).total_remaining_founds;
...
function run_simulation(input, initial_hour_rate, hour_rate_increment, onProgress) 
{
    var hour_rate = initial_hour_rate;
    var fi_values = new Array(input.founding_period_years);
    var fail_values = new Array(input.founding_period_years);
    ...
    while (true) 
    {
        ...
        var output = simulate_founding_period(sim_data);

        if(onProgress)
        {
            onProgress((input.founding_period_years - output.financial_independence) / input.founding_period_years);
        }
    ...
    return {
        ...
        fi_values: new Map(forbackd_fill(fi_values).map(function (hour_rate, year) { return [year, hour_rate]; })),
        fail_values: new Map(forbackd_fill(fail_values).map(function (hour_rate, year) { return [year, hour_rate]; }))
    };
}
